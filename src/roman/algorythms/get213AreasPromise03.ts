...
const calculateArea03 = async ({ shape, a, l, w, r, b, h }: GetAreaParam): Promise<string | number> => {
  switch (shape) {
    case 'square': {
      return new Promise((resolve, reject) => {
        try {
          if (a === undefined) reject(new Error('-1'))
          else resolve(Math.pow(a, 2).toFixed(2))
        } catch (error: unknown) {
          reject(new Error('-1'))
        }
      })
...
const getAreasPromise03 = async (params: GetAreaParam[]): Promise<(number | string)[]> => {
  try {
    return await Promise.all(
      params.map(async (item: GetAreaParam) => {
  ...
  ;(async () => {
    ...
    const output = await Promise.all(
      examples.map(async (example: ExampleType, index: number) => {
        ...
        const output = await getAreasPromise03(params)
