...
const getHash: GetHashType = ({ str, hashSize }) =>
  str.split('').reduce((accum: number, char: string, index: number) => {
    return (accum + index * 31 + char.charCodeAt(0)) % hashSize
  }, 0)
...
const getCreatedHashTable: GetCreatedHashTableType = ({ inputArray, hashSize }) => {
  const hashTable: any[] = Array.from({ length: hashSize }, () => [])

  inputArray.forEach((item: any) => {
    const hash = getHash({ str: item.id, hashSize })
    hashTable[hash].push(item)
  })
...
const getFound: GetFindType = ({ hashTabe, id, hashSize }) => {
  ...
  const bean = hashTabe[hashIndex]

  return bean.find((item: any) => item.id === id)
}
...
const get305SearchedHash01 = ({
  ...
  const hashTabe = getCreatedHashTable({ inputArray, hashSize })
  return getFound({ hashTabe, id: target, hashSize })
}
