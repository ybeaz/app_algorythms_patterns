const calculateArea02 = (shape: string, values: number[]): string => {
  let area
...
type GetAreasPromiseParams02Type = {
  shape: string
  values: number[]
}

type GetTemplateFuncRes02Type = string

const getAreasPromise02 = ({ shape, values }: GetAreasPromiseParams02Type): Promise<GetTemplateFuncRes02Type> => {
  return new Promise((resolve, reject) => {
    try {
      throw new Error('Something is wrong in try block') // It is not visible
      resolve(calculateArea02(shape, values))
    } catch (error) {
      reject(new Error(JSON.stringify({ message: 'We are in the catch block', cause: error })))
    }
  })
}
...
if (require.main === module) {
  ;(async () => {
    type ExampleType = {
      params: GetAreasPromiseParams02Type
      expected: GetTemplateFuncRes02Type
    }
    ...
    examples.map(async (example: ExampleType, index: number) => {
      ...
      const output = await getAreasPromise02(params)
