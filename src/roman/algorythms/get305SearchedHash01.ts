...
const getHash: GetHashType = ({ str, hashSize }) => {
  return str.split('').reduce((accum: number, char: string, strIndex: number) => {
    return (accum + strIndex * 31 + char.charCodeAt(0)) % hashSize
  }, 0)
}
...
const getCreatedHashTable: GetCreatedHashTableType = ({ inputArray, hashSize }) => {
  let output: any = Array.from({ length: hashSize }, () => [])

  inputArray.forEach((item: UserType) => {
    const { id } = item
    const hashIndex = getHash({ str: id, hashSize })
    output[hashIndex].push(item)
  })
  ...
  return inputArray.reduce(
    (accum: UserType[][], item: UserType) => {
      const { id } = item

      const hashIndex = getHash({ str: id, hashSize })
      accum[hashIndex].push(item)

      return accum
    },
    Array.from({ length: hashSize }, () => [])
  )
  */
}

interface GetFindType {
  (params: { hashArray: UserType[][]; id: string; hashSize: number }): UserType | undefined
}

const getFound: GetFindType = ({ hashArray, id, hashSize }) => {
  const hashIndex = getHash({ str: id, hashSize })
  const busket = hashArray[hashIndex]
  const output = busket.find((item: any) => item?.id === id)
...
const get305SearchedHash01 = ({
  ...
  const hashArray = getCreatedHashTable({ inputArray, hashSize })

  return getFound({ hashArray, id: target, hashSize })
}

/**
 * @description Here the file is being run directly
 * @run npx tsx src/roman/algorythms/get305SearchedHash01.ts
 */
...
const res = getHash({ str: 'abc', hashSize: 10 })
