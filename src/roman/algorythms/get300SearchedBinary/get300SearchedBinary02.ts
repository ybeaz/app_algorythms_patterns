...
type Get300BSearchedBinary02PropsType = {
  inputArray: number[]
  target: number
}

type Get300BSearchedBinary02ResType = number

const get300BSearchedBinary02 = ({
  inputArray,
  target,
}: Get300BSearchedBinary02PropsType): Get300BSearchedBinary02ResType => {
  let left = 0
  ...
  while (left <= right) {
    ...
    if (inputArray[mid] < target) left = mid
    else right = mid
  }
 ...
 * @run npx tsx src/roman/algorythms/get300BSearchedBinary02.ts
 */
if (require.main === module) {
  ;(async () => {
    type ExampleType = {
      params: Get300BSearchedBinary02PropsType
      expected: Get300BSearchedBinary02ResType
    }
    ...
    const output = await Promise.all(
      examples.map(async (example: ExampleType, index: number) => {
        ...
        const output = await get300BSearchedBinary02(params)
