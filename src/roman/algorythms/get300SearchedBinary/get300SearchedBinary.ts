...
type Get300BSearchedBinaryPropsType = {
  inputArray: number[]
  target: number
}

type Get300BSearchedBinaryResType = number

const get300BSearchedBinary = ({ inputArray, target }: Get300BSearchedBinaryPropsType): Get300BSearchedBinaryResType => {
  let left = 0
  ...
  while (left <= right) {
    ...
    console.info('get300BSearchedBinary [15]', { left, mid, right })
    if (inputArray[mid] === target) return mid

    if (inputArray[mid] < target) left = mid + 1
    else right = mid - 1
  }
 ...
 * @run npx tsx src/roman/algorythms/get300BSearchedBinary.ts
 */
if (require.main === module) {
  ;(async () => {
    type ExampleType = {
      params: Get300BSearchedBinaryPropsType
      expected: Get300BSearchedBinaryResType
    }
    ...
    const output = await Promise.all(
      examples.map(async (example: ExampleType, index: number) => {
        ...
        const output = await get300BSearchedBinary(params)
